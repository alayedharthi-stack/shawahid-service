...
══ قاعدة حرجة للأسماء العربية — لا استثناء ══
❗ لا تستخرج اسم المعلم من محتوى الوثائق المرفقة (PDF / صور / ملفات).
❗ اسم المعلم يُحدَّث فقط عندما يذكره صراحةً في رسالة نصية مباشرة ("اسمي ...").
عند استخراج الاسم (name) من تفريغ صوتي أو نص:
• استخدم الاسم حرفيًا كما ورد في التفريغ — لا تصحّح الإملاء ولا تغيّر الحروف.
• الأسماء العربية لها أشكال متعددة صحيحة: "عايد" ≠ "عائد"، "الحارثي" ≠ "الحارفي"، "حسين" ≠ "حسني".
...
• إذا كان الإدخال ملف PDF أو صورة → لا تضع أي قيمة في profile_update.name أبدًا.\
"""
...
══ التصنيفات الرسمية المسموحة (استخدم هذه حصرًا في حقل "category") ══
- التخطيط          ← للخطط الدراسية والأسبوعية والفصلية وتوزيع المنهج والتحضير
- سجل المتابعة     ← لسجلات حضور/غياب الطلاب ومتابعتهم اليومية
- التقويم          ← للاختبارات وأوراق العمل والواجبات والدرجات والمهام الأدائية
- ملفات إدارية     ← للتعاميم والخطابات الرسمية والقرارات الإدارية فقط
- الدورات والشهادات ← لشهادات التدريب والتطوير المهني
- التنفيذ داخل الصف ← للأنشطة الصفية والعروض التقديمية
- التعلم التعاوني
- التعلم بالممارسة
- التحفيز
...
- "الأهداف / نواتج التعلم / بنهاية الدرس يستطيع / تحضير / الأسبوع الأول / الوحدة الأولى" → التخطيط
- "توزيع منهج / توزيع الدروس / التوزيع الزمني / الفصل الدراسي / المقرر" → التخطيط
- "الدرجة الكلية / توزيع الدرجات / اختر / أجب / ورقة عمل / واجب" → التقويم
- "اختبار نهائي / مهمة أدائية / أسئلة الاختبار" → التقويم
- "اسم الطالب / رقم الطالب / الحضور / الغياب / سجل المتابعة" → سجل المتابعة
- "المعدل / التقدير / ممتاز / جيد جدًا / جيد / مقبول / كشف الدرجات" → التقويم
- "تعميم رقم / قرار رقم / المديرية العامة / نحيطكم علمًا" → ملفات إدارية
- "المشارك / يُشهد / اجتاز بنجاح / شهادة إتمام" → الدورات والشهادات

══ تمييز حاسم: جدول مدرسي ≠ خطة تعليمية ══
هذا الخطأ شائع — تعرّف على الفرق:

✅ خطة تعليمية / توزيع منهج (category = التخطيط):
  يحتوي: أسابيع + نواتج تعلم + موضوعات دروس + توزيع زمني
  أمثلة محتوى: "الأسبوع الأول - الوحدة الأولى - نواتج التعلم: يستطيع الطالب..."

✅ جدول مدرسي / جدول الحصص (category = ملفات إدارية):
  يحتوي: أيام الأسبوع (الأحد-الخميس) × أرقام الحصص (الأولى-السابعة)
  أمثلة محتوى: "الأحد - الحصة الأولى: رياضيات / الحصة الثانية: عربي..."

❗ القاعدة: إذا وجدت (الأحد + الاثنين + الثلاثاء) مع (الحصة الأولى + الثانية) → ملفات إدارية.
❗ القاعدة: إذا وجدت (نواتج التعلم + أسابيع + توزيع دروس) → التخطيط.

══ قواعد التحليل الحرجة ══
1. اعتمد على المحتوى أولاً، واسم الملف كدليل ثانوي فقط.
2. إذا لم تجد دليلاً كافياً → صنّف كـ "ملفات إدارية" بدلاً من التخمين.
3. العنوان الذكي يجب أن يكون وصفياً وليس اسم الملف الحرفي.
4. ❗ لا تستخرج اسم المعلم من محتوى الوثيقة — أبقِ حقل name فارغًا دائمًا.
5. لا تصحّح أسماء الأشخاص أو المدارس أو المناطق الواردة في الوثيقة.
6. إذا الوثيقة تخص مادة معينة، استخرجها في حقل subject.
7. ❗ لا تضع "ملفات إدارية" لأي خطة أسبوعية أو توزيع منهج أو اختبار — هذه تنتمي للتخطيط أو التقويم.
8. ❗ لا تصنّف جدول الحصص المدرسي (أيام × حصص) كـ "التخطيط" — فهو "ملفات إدارية".

أرجع JSON فقط بهذا الشكل:
{
  "document_type": "خطة درس | توزيع منهج | اختبار | ورقة عمل | سجل متابعة | كشف درجات | تقرير | تعميم | شهادة | مادة إثرائية | ملفات إدارية | أخرى",
  "category": "أحد التصنيفات الرسمية المسموحة أعلاه",
...
def analyze_pdf_document(
    *,
    ...
    # ── Auto-detect planning vs school-timetable from raw content ──────────────
    _ct = content.lower()
    _TIMETABLE_DAY_SET = {"الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس"}
    _TIMETABLE_PERIOD_SET = {"الحصة الأولى", "الحصة الثانية", "الحصة الثالثة",
                              "الحصة الرابعة", "الحصة الخامسة", "الحصة السادسة",
                              "الحصه الاولى", "الحصه الثانيه"}
    _PLANNING_SIGNALS = {"نواتج التعلم", "نواتج تعلم", "توزيع المنهج", "توزيع منهج",
                          "الأسبوع الأول", "الأسبوع الثاني", "التوزيع الزمني",
                          "الوحدة الأولى", "خطة أسبوعية", "خطة فصلية"}
    _day_hits    = sum(1 for d in _TIMETABLE_DAY_SET    if d in content)
    _period_hits = sum(1 for p in _TIMETABLE_PERIOD_SET if p in content)
    _plan_hits   = sum(1 for p in _PLANNING_SIGNALS     if p in content)
    _is_timetable = _day_hits >= 2 and _period_hits >= 1
    _is_planning  = _plan_hits >= 1
    ...
    if _is_timetable:       signals_parts.append("⚠️ يبدو جدولاً مدرسياً (أيام × حصص) → ملفات إدارية وليس تخطيط")
    if _is_planning:        signals_parts.append("✓ يحتوي مؤشرات خطة تعليمية (نواتج/توزيع/أسابيع)")
